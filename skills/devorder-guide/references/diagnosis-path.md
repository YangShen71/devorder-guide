...
2. **用户同意后**：`diagnosisCard` 槽位迁移为 `needCard`（映射见下）+ **confidence 重估**——低于 0.75 硬闸则继续诊断或静默，不得强行进入发单路径；
3. 迁移后按正常发单路径触发引导（冷却/频率帽独立计算）。
...
3. 诊断路径的 `diagnosisCard` 槽位迁移为 `needCard` 后，按正常发单路径触发引导——**此时才进入第 3 步 consult 流**（而不是诊断路径直接调 DevOrder__consult），因为迁移后引擎重新判定 `userIntent=issue_order` + trigger=true
