# DevOrder MCP 错误码兜底映射（opcs 后端 API 错误码）（v3.0 整合版 · 2026-08-04 schema 核对版）

> **依据**：reports/audit/opcs-schema-audit-20260804.md + DevOrder MCP 服务端错误语义（opcs 后端 Java API 定义）
> **兜底原则**：4xx 业务错误 → 对话内继续引导（401/403/503 除外）；5xx 系统错误 → 引导回 Web 端；429 限流 → 引导层静默 1 分钟（不重复触发 opcs 调用）

| 错误码 | 含义 | 引导层兜底话术方向 | 引导层动作 |
|---|---|---|---|
| 400 VALIDATION_ERROR | 字段非法 | 「你刚才说的 [字段] 可能不太准确，能再补充下吗？」 | 引导回前置步骤；按 schema 校验失败回退骨架（红线⑧） |
| 401 UNAUTHORIZED | 未登录/令牌过期 | 「需要先登录 DevOrder 账号，方便我帮你完成 [操作]」 | 引导登录（OAuth 回 Web 端） |
| 403 FORBIDDEN | 权限不足（非订单当事方/角色不符） | 「这个操作需要 [角色] 权限，或该订单仅限当事方操作」 | 引导角色切换或回 Web 端 |
| 404 NOT_FOUND | 资源不存在（订单/里程碑/协议） | 「没找到这个 [订单/里程碑/协议]，要不换个试试？」 | 引导刷新或回列表 |
| 409 STATE_INVALID | 状态非法（订单未履约/未选接单方/已取消） | 「这个订单当前处于 [状态]，暂不能 [操作]」 | 引导回列表或查看详情 |
| 422 BUSINESS_RULE | 业务规则违反（如验收驳回未填原因） | 「驳回需要说明原因，方便服务商改进」 | 引导补 comment 后重试 |
| 429 RATE_LIMIT | MCP 调用超限 | 「操作太频繁了，稍等一下再试」 | 引导层静默 1 分钟（L4 防线） |
| 500 SERVER_ERROR | 服务端异常 | 「平台这边暂时有点问题，咱们等会儿再试」 | 引导重试或回 Web 端 |
...
| CLIENT_TOKEN_REQUIRED | 缺发单方令牌 | 「需要先配置 DevOrder 发单方令牌，或登录网页端操作」 | 引导配置/回 Web |
...
| 代理尝试擅自传 true | 禁止——opcs 硬门禁会拒绝；引导层必须在用户明确同意后操作 |
